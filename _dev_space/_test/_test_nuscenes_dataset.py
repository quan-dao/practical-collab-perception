...
show_pointcloud(points[:, :3], gt_boxes, fgr_mask=points[:, -2] > -1)
...
fg_merge_ids = fg_points[:, -2].astype(int) * cfg.MAX_SWEEPS + fg_points[:, -3].astype(int)  # (N_fg, 3 + C)
...
show_pointcloud(_points[:, :3], gt_boxes, fgr_mask=_points[:, -2] > -1)
