...
def main(test_dataset: bool,
    ...
    if test_dataloader:
        ...
        for chosen_batch_idx in range(batch_dict['batch_size'] if 'batch_size' in batch_dict else batch_size):
            ...
            cur_boxes = cur_boxes[cur_boxes[:, -1] > 0]  # remove padded boxes
            # cur_boxes = batch_dict['metadata'][chosen_batch_idx]['pseudo_boxes']
            print(f'sample {chosen_batch_idx} | sampled_pseudo_boxes: {cur_boxes.shape}')
            if cur_boxes.shape[0] > 0:
                unq_pts_inst_idx = np.unique(cur_points[:, -1].astype(int))
                print(f'sample {chosen_batch_idx} | unq_pts_inst_idx: {unq_pts_inst_idx[1:]}')

                boxes_inst_idx = batch_dict['metadata'][chosen_batch_idx]['gt_inst_idx']
                print(f'sample {chosen_batch_idx} | boxes_inst_idx - {boxes_inst_idx.shape}: {boxes_inst_idx.astype(int)}')

                # TODO: assert all points has boxes
                for _i in unq_pts_inst_idx:
                    if _i == -1:
                        continue
                    if _i not in boxes_inst_idx:
                        print(f'warning {_i}')
                        painter = PointsPainter(xyz=cur_points[:, 1: 4], 
                                                boxes=cur_boxes[:, :7])
                        points_color = color_points_binary(cur_points[:, -1] == _i)
            ...
            # return
            # for inst_idx in unq_pts_inst_idx:
            #     if inst_idx == -1:
            #         continue
            #     print(f'\t inst_idx: {inst_idx}')
            #     painter = PointsPainter(xyz=cur_points[cur_points[:, -1].astype(int) == inst_idx, 1: 4], 
            #                             boxes=cur_boxes[boxes_inst_idx[:cur_boxes.shape[0]].astype(int) == inst_idx, :7])
            #     points_color = color_points_binary(cur_points[:, -1] > -1)
